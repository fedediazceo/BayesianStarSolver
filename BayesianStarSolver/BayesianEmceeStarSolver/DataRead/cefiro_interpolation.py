    ...
    def _interpolate_parameters(self):
        ...
        for M in masses:
            ...
            for param in self.parameters:
                ...
                # remove possible duplicated values generated by an error on the model maybe
                xcs_sliced = xcs[izams:itams]
                subgrid_sliced = subgrid[param].tolist()[izams:itams]

                unique_dict = {x: y for x, y in zip(xcs_sliced, subgrid_sliced)}

                unique_xcs = list(unique_dict.keys())
                corresponding_subgrid = list(unique_dict.values())

                f = interp1d(
                    unique_xcs, corresponding_subgrid, kind="quadratic", fill_value='extrapolate', bounds_error=False)
